...
function toAdd(task) {
    const exist = todos.some((todo) => todo.task.toLocaleLowerCase() === task.toLocaleLowerCase());
    if (exist) {
...
function edit(item) {
    ...
    const input = readline.question("Enter what you want to edit : task(Type T) or completed(Type C) ? : ");
    if (input === "T") {
...
function Menu() {
    ...
    while (!exit) {
        ...
        if (choice === "1") {
            ...
            const editinput = Number(readline.question("Which part you want to edit (Give id number of the task) : "));
            edit(editinput);
        }
        else if (choice === "4") {
